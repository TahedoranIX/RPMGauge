...
FILENAME_DEBUG = '/home/pi/CuentaRev/OBD.log'  # Nombre del archivo donde guardar la media de consumo
THROTTLE_MINIMUM = 7  # Min Throttle position
TIEMPO_RESET_CONS = 6  # Tiempo de reinicio de archivo con consumos 6*0.5(main) = 3s
"""
COMENTARIO:
PARA INICIAR EL PROGRAMA DE ARRANQUE:
$sudo crontab -e
y rellenar con (IMPORTANTE ORDEN):
@reboot rfcomm bind rfcommXX MAC_ADDRESS
@reboot python3 DIRECCION DE main.py partiendo desde raiz (/home/pi/CuentaRev/main.py)
"""
...
class Smart:
    def __init__(self, rs, en, d4, d5, d6, d7, port, e1, e2, eb, minimumSpeed, maxRev=5500, minRev=1200, debug=False, debugOBD=False):
        ...
        if debugOBD:
            ...
            fh = obd.logging.FileHandler(FILENAME_DEBUG)
            fh.setLevel(obd.logging.DEBUG)
    ...
    def __connection(self):
        try:
            ...
            connection = obd.OBD(self.__port)
